...
export default function AlertsManager() {
  ...
  const handleSaveNote = async (alertData, note) => {
    try {
      const res = await fetch(`${BASE_URL}/api/alerts/${alertData.id}/`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ notes: alertData.notes ? alertData.notes + "\n" + note : note }),
      });
      if (res.ok) {
        const updated = await res.json();
        handleUpdate(updated);
  ...
  const filtered = alerts.filter(a => {
    ...
    if (search) {
      ...
      const bev = (a.behavior_type || "").toLowerCase();
      if (!cam.includes(q) && !bev.includes(q)) return false;
    }
    return true;
  });
    ...
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginBottom: 20 }}>
        <div style={{ display: "flex", gap: 10 }}>
          {[
            { key: "ALL",            label: "All Alerts",    val: alerts.length,                                    icon: "⊞", cls: "ops-kpi--default" },
            { key: "NEW",            label: "New",           val: newCount,                                         icon: "●", cls: "ops-kpi--blue" },
            { key: "ESCALATED",      label: "Escalated",     val: alerts.filter(a => a.status === "ESCALATED").length,  icon: "⚠️", cls: "ops-kpi--red" },
        ...
        <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
          {newCount > 0 && (
            <div className="sg-chip sg-sev-CRITICAL" style={{
      ...
      <div style={{ display: "flex", gap: 14, alignItems: "flex-start" }}>
        ...
        <div className="sg-card" style={{ flex: 1, minWidth: 0 }}>
          ...
          {loading ? (
            ...
            <div className="sg-empty" style={{ color: "var(--accent-red)" }}>⚠️ {error}</div>
          ) : filtered.length === 0 ? (
      ...
      <div className="ops-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="ops-modal-body">
          <div style={{ marginBottom: 12 }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text-muted)" }}>{time} · {camName}</div>
          </div>
          <textarea className="ops-modal-textarea" rows={4}
            placeholder="Add context or details..."
            value={note} onChange={e => setNote(e.target.value)} />
        </div>
        <div className="ops-modal-footer">
          <button className="det-btn-full det-btn-full--primary" onClick={() => onSave(alert, note)}>Save Note</button>
